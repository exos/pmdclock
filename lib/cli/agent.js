...
module.exports = function (args) {
    ...
    server.addCmd("start", (name, cmode = true) => {
        ...
        if (actual && cmode && (actual.type === Session.types.TYPE_REST ||
                         actual.type === Session.types.TYPE_LARGE_REST)) {
            actual.finish(); 
    ...
    server.addCmd("rest", (type, cmode = true) => {
        var session;

        var actual = Session.thereAreActual();

        if (actual && cmode && actual.type === Session.types.TYPE_TASK) {
            actual.finish(); 
            History.getInstance().save(actual);
        }
