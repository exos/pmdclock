...
program
    ...
    .option("-h, --head <n>", `Limit of results at the first x`, parseInt)
    .option("-t, --tail <n>", `Limit of result at the last x`, parseInt)
    .option("-r --reverse", `Reverse order, show first the last registers`)
    .action(list);
...
program
    ...
    .action(() => {
        process.stderr.write(`The command ${program.args[0]}, is not a valid ` +
                             `pmdclock command, see ${program.name()} --help`);
        process.exit(1);
    });
